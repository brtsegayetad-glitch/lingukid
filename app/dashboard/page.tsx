...
export default function ParentDashboard() {
  ...
  return (
    <div className="min-h-screen bg-white pb-20"> {/* Padding bottom for the logout area */}
      <div className="max-w-6xl mx-auto p-4 md:p-8">
        
        {/* HEADER SECTION */}
        <header className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-3xl md:text-6xl font-black uppercase italic tracking-tighter leading-none">Parent</h1>
            <h2 className="text-xl font-bold uppercase text-blue-600 tracking-widest">Dashboard</h2>
          </div>
          <button 
            onClick={() => router.push('/dashboard/find-tutor')} 
            className="bg-black text-white px-4 py-2 rounded-xl font-black uppercase text-[9px] md:text-[10px] shadow-[3px_3px_0px_0px_rgba(37,99,235,1)] active:translate-y-1 active:shadow-none transition-all"
          >
            Tutors & Packages
          </button>
        </header>

        {/* 1. STUDENT REGISTRATION (TOP PRIORITY ON MOBILE) */}
        <section className="mb-6">
          <div className="bg-blue-50 p-5 md:p-8 rounded-[30px] border-4 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-black uppercase italic">My Children</h3>
              <button 
        ...
        <section className="mb-8">
          <div className="bg-green-500 p-4 rounded-2xl text-white border-4 border-black flex items-center justify-between shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
            <div>
              ...
              <p className="text-2xl font-black italic uppercase leading-none">Lesson Hours</p>
            </div>
            <div className="bg-white text-green-600 px-4 py-1 rounded-xl border-2 border-black">
        ...
        {/* 3. SCHEDULE SECTION */}
        <section className="bg-white border-4 border-black p-5 md:p-8 rounded-[30px] shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
          <h2 className="text-xl md:text-3xl font-black uppercase italic mb-6 underline decoration-blue-500 decoration-4 underline-offset-4">Upcoming Lessons</h2>
          
          <div className="space-y-3">
            {bookings.length > 0 ? (
              bookings.map(b => (
                <div key={b.id} className="flex flex-col p-4 border-2 border-gray-100 rounded-2xl hover:border-blue-500 transition-colors gap-2 bg-gray-50/50">
                   ...
                   <div className="flex items-center justify-between mt-1">
                      <p className="text-[9px] font-bold text-blue-600 uppercase">Tutor: {b.tutors?.full_name}</p>
                      {b.zoom_link ? (
                        <a href={b.zoom_link} target="_blank" rel="noopener noreferrer" className="bg-blue-600 text-white px-3 py-1.5 rounded-lg font-black text-[8px] uppercase border-2 border-black">Join Class</a>
                      ) : (
        ...
        <footer className="mt-12 mb-6 text-center">
          <button 
            ...
            className="w-full max-w-xs mx-auto bg-gray-100 text-gray-500 py-3 rounded-xl font-black uppercase text-[10px] border-2 border-gray-200 hover:bg-red-50 hover:text-red-600 hover:border-red-600 transition-all active:scale-95"
          >
            Logout from Dashboard
          </button>
          <p className="mt-4 text-[8px] font-bold text-gray-300 uppercase tracking-widest">LinguKid © 2026</p>
        </footer>
